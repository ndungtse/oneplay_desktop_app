...
const Navbar = () => {
    ...
    if(!isMaximized)
		  global.ipcRenderer.send("maximize");
    else
      global.ipcRenderer.send("unmaximize");
	};
	...
	return (
		<div className=' z-[50] flex h-[50px] w-full items-center justify-between bg-[#030305]/90 border-b-2 border-main/20 backdrop-blur-lg p-3 px-0 shadow-xl'>
			...
			<div className='flex items-center'>
				...
				<div className='flex ml-5 items-center z-50 h-full'>
					<button
						onClick={minimize}
            title='Minimize'
						className=' p-2 hover:bg-slate-400/40 cursor-pointer h-[50px] w-[50px] items-center justify-center flex'
					>
						<BiMinus />
					</button>
					<button
            ...
            {isMaximized? <VscChromeRestore /> : <BiRectangle />}
					...
					<button
						onClick={close}
            title='Close To Tray'
						className=' p-1 hover:bg-red-600 cursor-pointer w-[50px] h-[50px] items-center justify-center flex'
					>
						<BiX size={24} />
					</button>
				</div>
